...
cfg = {
    "script_name": 'opt_train_dart_op_154_0',
...
paths = ['../input/opt-train-dart-nested-op-175-0', '../input/opt-train-dart-nested-op-175-1', '../input/opt-train-dart-nested-op-175-2', '../input/opt-train-dart-nested-op-175-3', '../input/opt-train-dart-nested-op-175-4']
import shutil
train = pd.read_pickle(cfg['path_features'])
oof_preds = np.zeros(train.shape[0])
for fold, path in zip([0, 1, 2, 3, 4], paths):
    for file in os.listdir(path): 
        if file.startswith('dart'): 
            print(file)
            shutil.copy(os.path.join(path, file), file)
    mask = train.fold == fold
    oof_preds[mask] = np.load(os.path.join(path, 'oof_predictions.npy'))[mask]
np.save('oof_predictions', oof_preds)
print(f'oof_score: {rmspe(train.target, oof_preds)}')
...
def main(): 
    ...
    for f in [OUTER_FOLD]:
        cfg['prefix'] = f'dart_{f}_'
        test_ind = x.fold == f
        x_test, y_test = x[test_ind].drop('fold', axis=1), y[test_ind]
        x_test = encode_cols(x_test, 
                              cfg['encode_stock_cols'], 
                              funcs=cfg['encode_funcs'], 
                              shake=cfg['shake'], 
                              shake_std=cfg['shake_std']).drop('stock_id', axis=1)
        
        for fold in [i for i in range(5) if i != f]:
            print(f"outer/inner fold: {f, fold}")
            trn_ind = (x.fold != fold) & (x.fold != f)
            val_ind = x.fold == fold
            ...
            oof_predictions[test_ind] += y_pred / 4
            scores.append(round(rmspe(y_test, y_pred), 3))
    ...
    for fold in range(len(booster_summaries)):
